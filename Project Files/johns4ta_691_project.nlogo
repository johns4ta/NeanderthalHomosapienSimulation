...
;Once the user presses go to start the simulation, this function is called and the contents of the function are looped over until the
...
;a person is not at their target yet, then they move towards their target, with the distance they move towards their target being 
...
;new resources are populated into the world, and average values of traits are calculated for the purpose of plotting.
...
;determineTarget is called to determine what the inital target the Neanderthal will head towards is.
...
;to form their genes by filling in their bit array values. Finally, calculateAttributes is called to calculate their traits and set them, and 
;determineTarget is called to determine what the inital target the homosapien will head towards is.
